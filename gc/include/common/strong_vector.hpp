...
struct StrongVector
{
    ...
    explicit StrongVector(Count size) :
        v(size.v)
    {}

    explicit StrongVector(Count size, const value_type& init) :
        v(size.v, init)
    {}

    explicit StrongVector(Weak v) :
        v(std::move(v))
    {}
    ...
    auto begin() const noexcept -> const_iterator { return v.begin(); }
    auto end() const noexcept -> const_iterator { return v.end(); }
    auto cbegin() const noexcept -> const_iterator { return v.begin(); }
    auto cend() const noexcept -> const_iterator { return v.end(); }

    auto rbegin() noexcept -> iterator { return v.rbegin(); }
    auto rend() noexcept -> iterator { return v.rend(); }
    auto rbegin() const noexcept -> const_iterator { return v.rbegin(); }
    auto rend() const noexcept -> const_iterator { return v.rend(); }
    ...
    auto empty() const noexcept -> bool { return v.empty(); }
    auto size() const noexcept -> Count { return checked_count_(v.size()); }
    auto capacity() const noexcept -> Count
    { return checked_count_(v.capacity()); }
    auto reserve(Count c) -> void { v.reserve(c.v); }

    auto clear() noexcept -> void { v.clear(); }
    // TODO: insert
    auto push_back(const V& element) -> void { v.push_back(element); }
    auto push_back(V&& element) -> void { v.push_back(std::move(element)); }

    template <typename... Args>
    auto emplace_back(Args&&... args) -> V&
    { return v.emplace_back(std::forward<Args>(args)...); }

    auto pop_back() -> void { v.pop_back(); }

    auto resize(Count c) -> void { v.resize(c.v); }
    auto resize(Count c, const V& init) -> void { v.resize(c.v, init); }
    auto swap(StrongVector& that) -> void { v.swap(that.v); }

    constexpr auto index_range() const noexcept
        -> IndexRange<I>
    { return IndexRange<I>(size()); }

    auto operator==(const StrongVector& that) const noexcept -> bool
    { return v == that.v; }

    auto operator<=>(const StrongVector& that) const noexcept
    { return v <=> that.v; }
