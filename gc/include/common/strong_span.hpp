...
struct StrongSpan
{
    ...
    explicit StrongSpan(It first, Count size) :
        v{first, size.v}
    {}

    template <typename It, typename End>
    requires (Extent == std::dynamic_extent)
    explicit StrongSpan(It first, End end) :
        v{first, end}
    {}

    template< std::size_t N >
    requires (Extent == std::dynamic_extent || Extent == N)
    constexpr StrongSpan(std::type_identity_t<V> (&arr)[N] ) noexcept :
        v{arr}
    {}

    template< typename U, std::size_t N >
    requires (Extent == std::dynamic_extent || Extent == N)
    constexpr StrongSpan(std::array<U, N>& arr) noexcept :
        v{arr}
    {}

    template< typename U, std::size_t N >
    requires (Extent == std::dynamic_extent || Extent == N)
    constexpr StrongSpan(const std::array<U, N>& arr) noexcept :
        v{arr}
    {}

    template< class R >
    requires(Extent == std::dynamic_extent)
    constexpr StrongSpan( R&& range ) :
        v( std::forward<R>(range) )
    {}

    explicit StrongSpan(Weak v) :
        v(v)
    {}
    ...
    auto begin() const noexcept -> iterator { return v.begin(); }
    auto end() const noexcept -> iterator { return v.end(); }

    auto rbegin() const noexcept -> iterator { return v.rbegin(); }
    auto rend() const noexcept -> iterator { return v.rend(); }
    ...
    constexpr auto subspan(Count size) const
        -> DynamicStrongSpan
    { return DynamicStrongSpan{ v.subspan(0, size.v) }; }

    constexpr auto subspan(Index offset, Count count) const
        -> DynamicStrongSpan
    ...
    auto operator==(const StrongSpan& that) const noexcept -> bool
    { return v == that.v; }

    auto operator<=>(const StrongSpan& that) const noexcept
    { return v <=> that.v; }
