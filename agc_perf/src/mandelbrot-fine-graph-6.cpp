...
auto activate_Replicate_trigger_z0(State* s)
    -> void
{ activate_Iterator_init(s, &s->z0.value); }

auto activate_Replicate_trigger_iter_val(State* s)
    -> void
{ activate_Mag2(s, &s->f_iter.z); }

auto activate_Replicate_trigger_iter_count(State* s)
    -> void
{ activate_Scale_scale(s, &s->iter_count.n); }
 ...
 *  first argument of type `MandelbrotFineGraphImpl5_Tag`. The change done
 *  in this implementatoin is the reduction of copying pointers. Specifically,
 *  we no longer copy the following pointers:
 *  - to the current grid value (aka c)
 *  - to all replicator values (z0, z, n)
 *
 *  Benchmarks show 1.04x slowdown with resp. to the reference implementation.
 */
auto mandelbrot_set(MandelbrotFineGraphImpl6_Tag, const MandelbrotParam& param)
    -> Canvas
