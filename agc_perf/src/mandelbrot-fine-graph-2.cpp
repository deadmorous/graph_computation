...
auto activate_F_arg(State* s, const Complex* arg)
    -> void
{
    s->f.f_z = *arg * *arg + *s->f.c;
    activate_Iterator_f(s, &s->f.f_z);
}

auto activate_Mag2(State* s, const Complex* z)
    -> void
{
    s->iter_val_mag2.mag2 = z->real()*z->real() + z->imag()*z->imag();
    activate_Threshold_check_mag2(s, &s->iter_val_mag2.mag2);
}
...
auto activate_Scale_scale(State* s, const uint64_t* in)
    -> void
{
    s->result_scale.result = static_cast<double>(*in * *s->result_scale.factor);
    activate_Canvas_next(s, &s->result_scale.result);
}
