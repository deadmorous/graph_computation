 ...
 *  in this implementatoin is reducing the number of data pointers: we prefer
 *  copying values to copying pointers.
 *
 *  Benchmarks show 1.81x slowdown with resp. to the reference implementation,
 *  so this version is slower than `MandelbrotFineGraphImpl3_Tag`. An
 *  explanation to that could be a greater size of `Complex` being copied than
 *  the size of pointer.
 */
auto mandelbrot_set(MandelbrotFineGraphImpl4_Tag, const MandelbrotParam& param)
    -> Canvas
